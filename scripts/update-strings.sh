...
do
  TRANSLATION_FILE="google_maps_geocoder/i18n/$LOCALE.ts"
...
then
  ...
  for LOCALE in ${LOCALES}
  do
    echo "google_maps_geocoder/i18n/"${LOCALE}".ts"
    # Note we don't use pylupdate with qt .pro file approach as it is flakey
    # about what is made available.
    ${PYLUPDATE} -noobsolete ${PYTHON_FILES} -ts cartography_tools/i18n/${LOCALE}.ts
  done
else
